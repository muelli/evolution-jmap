...
//! longest, but not the ones after it: [`Client`] takes its [`CancelFlag`] when
//! it is built and offers no way to re-point it, so a `GCancellable` handed to
//! `list_existing_sync` is currently observed by nobody. Making that work needs
//! a resettable flag shared between the client and a per-operation
//! [`CancelBridge`], which is a change to `jmap-client` rather than to this
//! file.
//!
