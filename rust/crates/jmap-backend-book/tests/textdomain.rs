...
//! That the module binds this project's gettext domain when EDS loads it.
//!
//! `evolution-addressbook-factory` has never heard of this project, so nothing
//! in that process has told gettext where our catalogues are. Until something
...
//! string. There is no diagnostic for that: a missing catalogue and a
//! catalogue with no translation for the message look identical.
//!
//! The binding therefore has to be made by the only code of ours that is
//! guaranteed to have run before any string is asked for: the entry point EDS
//! dlopened the module for. Doing it lazily at the first lookup would be a
//! shorter argument if we owned every lookup, and we do not — the strings this
//! backend will hand back are in `GError`s that Evolution displays, and by then
//! the text has already been chosen.
//!
...
unsafe impl ObjectSubclass for TestModule {
    const NAME: &'static CStr = c"JmapBookTextdomainTestModule";
...
/// because that is the path EDS takes and it is no harder to write: the entry
/// point is reached through the `GTypeModuleClass.load` vfunc, exactly as it
/// would be from `EModule`.
#[test]
fn the_entry_point_binds_the_domain() {
    let decoy = c"/nonexistent/jmap-book-decoy-locale";
    assert_eq!(
    ...
    // given back, which is what EDS does for as long as the backend is wanted.
    assert_ne!(
    ...
    assert_eq!(
        ...
        "the entry point did not bind the domain, so this backend's translated \
         strings would be looked for wherever the address book factory happened \
         to point"
    );
}
