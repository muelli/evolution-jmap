...
#[test]
fn address_book_share_can_be_widened_then_revoked() {
    let bob = Id::new("P-bob");
    let server = MockServer::builder()
        .bearer_token("alice-token")
        .bearer_token_as("bob-token", bob.clone())
    ...
    alice
        ...
        .unwrap();
    let rights = bob_client.address_books(&account_id).unwrap()[0]
        .my_rights
        .clone()
        .expect("myRights is computed from the grant");
    assert_eq!(rights.may_read, Some(true));
    assert_ne!(rights.may_write, Some(true), "mayWrite was not granted yet");
    ...
    alice
        ...
        .unwrap();
    let widened = bob_client.address_books(&account_id).unwrap()[0]
        .my_rights
        .clone()
        .expect("myRights is computed from the widened grant");
    assert_eq!(widened.may_read, Some(true));
    assert_eq!(widened.may_write, Some(true), "widened to mayWrite too");
