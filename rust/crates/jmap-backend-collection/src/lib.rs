...
//! which is where the fan-out actually happens, and the `ESourceCollection`
//! reads it needs. `dup_resource_id` comes first because `populate` cannot be
//! written without it — EDS loads the cached children and asks their resource
//! ids *before* it calls `populate`, and a populate that ran against a
//! mis-loaded child list would create duplicates of children that are already
//! there.
//!
