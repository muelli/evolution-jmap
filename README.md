...
**All four backends work end-to-end in real Evolution and have been
operator-verified against a real JMAP server.**

| Component | Crate(s) |
|---|---|
| JMAP protocol types | `evolution-jmap-proto` |
| Blocking JMAP client | `evolution-jmap-client` |
...
Evolution — not just against the mock. The mapping crates
(`jmap-vcard`/`jmap-ical`) carry extensive round-trip and fuzz tests; a
measured spike into externalising that layer onto the `calcard` crate's own
converter found it a worse fit against our acceptance suite (see
...
a real EDS registry and GUI-smoke tests against a real Evolution/Xvfb.
`ctest --test-dir build` reports the count.
...
The modules also install as one Debian package, built from that same
install tree:
...
rather than written down, so the package pins the Evolution and EDS ABI it
was compiled against. That is the contract for modules Evolution and EDS
dlopen: build them against the versions they will be installed alongside.
`ctest -R package-deb` builds the package and checks its contents and
control fields.
...
- One definition of the build, run everywhere: the [`ci/`](ci/) scripts
  ...
  everywhere and the platforms can't drift.
