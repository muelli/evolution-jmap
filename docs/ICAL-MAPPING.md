...
### 13.208 Divergence 208: `by_set_position_part`, `set_position_token`, `first_day_of_week_part`, and `weekday_token`: Outbound `BYSETPOS` and `WKST` Recurrence Rule Part Generation: Set Selection Gating (`selects_from_a_set`), Set Position Bounding (`-366..=-1 | 1..=366`), Zero Rejection, Monday Default Suppression (`WKST=MO`), and Lowercase Weekday Spelling Normalization

- **Observed Behavior**:
  Serializing recurrence rule set positions and week start boundaries into RFC 5545 `RRULE` syntax requires enforcing grammar prerequisites, numeric bounds, and default value suppression. In `jmap-ical`:
  1. `BYSETPOS` set selection dependency gating (`by_set_position_part`, `selects_from_a_set`): RFC 5545 Section 3.3.10 dictates that `BYSETPOS` operates solely on the occurrences generated by other `BYxxx` rule parts within an interval. `selects_from_a_set` checks whether any other `BYxxx` part is present (`by_second`, `by_minute`, `by_hour`, `by_day`, `by_month_day`, `by_year_day`, `by_week_no`, `by_month`). If no other `BYxxx` part is active, `by_set_position_part` returns `None`. Without other parts, `BYSETPOS=1` is a redundant no-op and `BYSETPOS=2` creates an empty series that never recurs; omitting the part signals `maps_recurrence_rule` that the recurrence was seen in part.
  2. Set position bounding and zero rejection (`set_position_token`): RFC 5545 defines `setposday` identically to `yeardaynum`, permitting `1` to `366` (and negative offsets `-366` to `-1` counting backwards). `set_position_token` enforces `-366..=-1 | 1..=366`. Position `0` is rejected because day intervals have no zeroth occurrence. Empty `by_set_position` arrays return `None`.
  3. Monday default suppression on `WKST` (`first_day_of_week_part`): RFC 5545 Section 3.3.10 specifies Monday (`MO`) as the default first day of the week. libical strips `WKST=MO` from any rule it parses. A rule emitted with `WKST=MO` would emerge from the Evolution Data Server cache without it, causing the save path to interpret the missing token as a user deletion of `firstDayOfWeek`. `first_day_of_week_part` suppresses `WKST=MO` while emitting non-default days (such as `WKST=SU`).
  4. Lowercase weekday tokenization (`weekday_token`): In JSCalendar (RFC 8984 Section 4.3.3), `firstDayOfWeek` requires two-letter lowercase tokens (`mo`, `tu`, `we`, `th`, `fr`, `sa`, `su`). `weekday_token` enforces ASCII lowercase representation and validates against the known table, rejecting uppercase or full-name strings.
  5. In contrast, differential oracles or permissive serializers emit `WKST=MO` explicitly (causing spurious EDS diff invalidations), admit position `0` in `BYSETPOS` (generating unparseable recurrence rules), or emit `BYSETPOS` without accompanying `BYxxx` parts.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.3.10 governs `BYSETPOS` dependencies and `WKST` defaults. RFC 8984 Section 4.3.3 defines JSCalendar `bySetPosition` and `firstDayOfWeek`.
  2. Suppressing default `WKST=MO` and gating `BYSETPOS` on active set generators prevents recurrence rule corruption and cache round-trip churn.
- **Adjudication**:
  Conforming specification boundary and recurrence rule part generation fidelity. Enforces set selection prerequisites for `BYSETPOS`, bounds position numbers while rejecting zero, suppresses default `WKST=MO`, and requires canonical lowercase weekday tokens.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.209 Divergence 209: `drawn_conferences`, `drawn_conference`, and `joining_features`: Outbound RFC 7986 `CONFERENCE` Virtual Meeting Serialization: Mandatory `VALUE=URI` Parameter Attachment, Multi-Endpoint Map Iteration, Stable Round-Trip Key Preservation (`X-JMAP-KEY`), Feature Parameter Decoding, and Label Parameter Binding (`LABEL`)

- **Observed Behavior**:
  Serializing virtual meetings and online conference endpoints into iCalendar format requires translating RFC 8984 `virtualLocations` into RFC 7986 `CONFERENCE` lines. In `jmap-ical`:
  1. Mandatory `VALUE=URI` parameter attachment: RFC 7986 Section 5.11 defines the grammar of `CONFERENCE`, making the `VALUE=URI` parameter mandatory on URI-based conference lines. Even though URI is the default value type, `drawn_conference` explicitly attaches `VALUE=URI` so that strict RFC 7986 consumers do not reject the entry.
  2. Multi-endpoint map iteration (`drawn_conferences`): Unlike RFC 5545 physical `LOCATION` (which permits only a single primary entry), RFC 7986 Section 5.11 allows multiple `CONFERENCE` lines. `drawn_conferences` iterates through all entries in the `virtual_locations` map in deterministic map order, preserving all meeting endpoints.
  3. Stable round-trip key preservation (`X-JMAP-KEY`): Each generated `CONFERENCE` line is tagged with `X-JMAP-KEY=<key>`. When the user edits or removes a conference in a client, the server patch modifies `virtualLocations/<key>/uri`. Positional matching cannot serve this purpose, because dropping an unsupported line in an external editor would misalign all subsequent conference entries.
  4. Feature parameter decoding (`joining_features`): JSCalendar boolean features (`audio`, `chat`, `feed`, `moderator`, `phone`, `screen`, `video`) are decoded and converted into uppercase RFC 7986 Section 6.3 `FEATURE` parameters (`AUDIO`, `CHAT`, `FEED`, `MODERATOR`, `PHONE`, `SCREEN`, `VIDEO`). Only features explicitly set to `true` are emitted; `false` or unlisted features are omitted.
  5. Label binding and URI gating: The optional JSCalendar `name` is emitted as parameter `LABEL=<name>`. Entries lacking a valid URI per `names_a_uri` are dropped entirely, preventing corrupted conference lines from entering the output stream.
  6. In contrast, differential oracles or naive converters omit mandatory `VALUE=URI` parameters, drop `X-JMAP-KEY` metadata (breaking granular patch synchronization), collapse multiple virtual locations into a single description field, or omit standardized conference feature parameters.
- **Specification and Architectural Context**:
  1. RFC 7986 Section 5.11 and Section 6.3 define `CONFERENCE`, `VALUE=URI`, `LABEL`, and feature tokens. RFC 8984 Section 4.2.6 defines JSCalendar `virtualLocations`.
  2. Explicit `VALUE=URI` emission and `X-JMAP-KEY` parameter preservation guarantee interoperability across RFC 7986 parsers and stateful JMAP patch engines.
- **Adjudication**:
  Conforming specification boundary and virtual conference serialization determinism. Emits mandatory `VALUE=URI`, preserves all endpoints with `X-JMAP-KEY`, maps RFC 7986 `FEATURE` parameters, binds `LABEL`, and gates on valid URIs.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.210 Divergence 210: `drawn_links`, `drawn_link`, `media_type`, `restricted_name`, and `stated_size`: Outbound External Resource Serialization: Disambiguation of Document Attachments (`ATTACH`) vs Icon Graphics (`IMAGE`), Mandatory `VALUE=URI` on Images, Display Mode Parameterization (`DISPLAY`), Media Type Parameterization (`FMTTYPE`), RFC 8607 Byte Size Parameterization (`SIZE`), and Stable Map Key Binding (`X-JMAP-KEY`)

- **Observed Behavior**:
  Translating RFC 8984 `links` into iCalendar requires disambiguating document attachments from visual graphics while parameterizing media metadata. In `jmap-ical`:
  1. Disambiguation of `ATTACH` vs `IMAGE`: RFC 8984 Section 4.2.7 unifies external references under `links`. In `drawn_link`, entries with `rel: "icon"` are emitted as RFC 7986 Section 5.10 `IMAGE` properties, while all other links are emitted as RFC 5545 Section 3.8.1.1 `ATTACH` entries.
  2. Mandatory `VALUE=URI` and display modes on `IMAGE`: RFC 7986 Section 5.10 requires `VALUE=URI` on URI-based image entries. `drawn_link` attaches `VALUE=URI` to all emitted `IMAGE` lines. Furthermore, it maps JSCalendar `display` to RFC 7986 Section 6.1 `DISPLAY` parameters (`BADGE`, `GRAPHIC`, `FULLSIZE`, `THUMBNAIL`). Conversely, `ATTACH` properties omit `VALUE=URI` (since URI is the implicit RFC 5545 default) and forbid `DISPLAY`.
  3. Media type validation and parameterization (`media_type`, `restricted_name`): Translates `contentType` into the `FMTTYPE` parameter. RFC 5545 Section 3.2.8 requires `FMTTYPE` to consist strictly of a type-name, a slash, and a subtype-name conforming to RFC 6838 Section 4.2 restricted names. Types carrying parameters (such as `text/plain; charset=utf-8`) or invalid characters are omitted at the parameter level without dropping the resource URI itself.
  4. RFC 8607 byte size parameterization (`stated_size`): If the link carries an integer `size` in bytes, it is serialized as `SIZE=<u64>` on `ATTACH` lines per RFC 8607 Section 4.1.
  5. Stable map key preservation (`X-JMAP-KEY`): Emits `X-JMAP-KEY=<key>` on each link entry, allowing patch-based synchronization to identify the exact link being modified. Entries with invalid or missing URIs are dropped.
  6. In contrast, differential oracles or standard JMAP serializers omit `SIZE`, emit `ATTACH` for icon graphics (losing display metadata), drop `X-JMAP-KEY` identifiers, or allow malformed media type parameters with semicolon characters to break iCalendar parameter quoting.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.2.8 and Section 3.8.1.1 govern `ATTACH` and `FMTTYPE`. RFC 7986 Section 5.10 and Section 6.1 govern `IMAGE` and `DISPLAY`. RFC 8607 Section 4.1 governs `SIZE`. RFC 8984 Section 4.2.7 defines JSCalendar `links`.
  2. Disambiguating image graphics from document attachments and preserving stable keys ensures faithful rendering and patch idempotence across clients.
- **Adjudication**:
  Conforming specification boundary and external resource serialization fidelity. Disambiguates `IMAGE` from `ATTACH`, attaches mandatory `VALUE=URI` and `DISPLAY` to images, encodes RFC 8607 `SIZE` on attachments, validates `FMTTYPE` restricted names, and preserves `X-JMAP-KEY`.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.211 Divergence 211: `parse_ical`, `check_structure`, `check_depth`, and `MAX_DEPTH`: Inbound iCalendar Envelope Parsing, Structure Validation, and Recursion Depth Defense: Byte Order Mark Stripping (`\u{feff}`), Matching `BEGIN`/`END` Balancing, Mismatched/Unterminated Component Detection, Trailing Garbage Rejection, Root `VCALENDAR` Type Enforcement, and Strict Nesting Limit Gating (`MAX_DEPTH = 32`)

- **Observed Behavior**:
  Ingesting untrusted iCalendar text requires defending against malformed component structures, unmatched delimiters, trailing payloads, and stack exhaustion attacks. In `jmap-ical`:
  1. Byte Order Mark stripping: `check_structure` inspects the input text and strips an optional UTF-8 Byte Order Mark (`\u{feff}`) before unfolding content lines, preventing the BOM from corrupting the initial `BEGIN:VCALENDAR` token.
  2. Component envelope delimiter balancing (`check_structure`): Tracks open component names on a stack across unfolded lines. Case-insensitively pairs `BEGIN:<name>` with `END:<name>`. If an `END` statement specifies a name differing from the innermost open component, it halts immediately with `ICalError::Mismatched { expected, found }`. If unclosed components remain at EOF, it returns `ICalError::Unterminated(name)`.
  3. Root `VCALENDAR` type enforcement: `parse_ical` inspects the outermost component parsed by calcard. If the root component is absent or its type is not `VCALENDAR` (case-insensitively), it rejects the document with `ICalError::NotACalendar`.
  4. Trailing garbage rejection: Following extraction of the primary calendar component, `parse_ical` verifies that the parser encountered `Entry::Eof`. Any subsequent token (such as trailing text, a secondary `VCALENDAR`, or invalid lines) returns `ICalError::Trailing`, preventing smuggling of unverified secondary objects.
  5. Component recursion depth gating (`check_depth`, `MAX_DEPTH`): Evaluates child component hierarchies using a traversal stack. If component nesting exceeds `MAX_DEPTH` (32 levels), traversal halts with `ICalError::TooDeep(component_type)`. This defends against deeply nested malicious payloads designed to cause call stack exhaustion or exponential memory consumption.
  6. In contrast, differential oracles or unhardened parsers permit arbitrary nesting depth, ignore trailing content or multiple root components, or produce undefined AST states when encountering mismatched `BEGIN`/`END` tags.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.4 defines iCalendar component envelope and nesting rules.
  2. Enforcing matching component pairs, rejecting trailing garbage, and capping recursion depth guarantees parser determinism and guards against denial-of-service vulnerabilities.
- **Adjudication**:
  Conforming specification boundary and parser structure hardening. Strips UTF-8 BOM, validates matching `BEGIN`/`END` envelopes, rejects non-VCALENDAR roots and trailing content, and enforces `MAX_DEPTH = 32` recursion limits.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.
...
### 13.232 Divergence 232: `shows_without_time`, `instance_shows_without_time`, `at_midnight`, `whole_days`, and `names_a_time_of_day`: Outbound All-Day `VALUE=DATE` Serialization Invariants: Document-Wide Date-Type Consensus, Midnight Start Requirement (`at_midnight`), Duration in Whole Days (`whole_days`), Sub-Day Recurrence Time Exclusion (`names_a_time_of_day`), and Recurrence Override Instance Date-Time Alignment

- **Observed Behavior**:
  Serializing all-day calendar events to RFC 5545 requires coordinating date representations across the entire component tree: `DTSTART`, `DURATION`, `RRULE:UNTIL`, `EXDATE`, `RDATE`, and detached `RECURRENCE-ID` components must all agree on whether the event is date-valued or date-time-valued. In `jmap-ical`:
  1. Document-wide date-type consensus: `shows_without_time` gates whether an event is emitted with `DTSTART;VALUE=DATE:YYYYMMDD` without a time component. RFC 8984 Section 4.1.5 specifies that `showWithoutTime: true` indicates an all-day event, but an event can only be written in this form if all related properties conform to date-only semantics.
  2. Midnight start requirement (`at_midnight`): The event's start timestamp must fall exactly at midnight (`T000000`). If an event with `showWithoutTime: true` specifies a non-midnight start time (such as 09:00), `shows_without_time` evaluates to `false`, emitting the event with date-time precision rather than truncating and silently shifting the event.
  3. Time zone omission: An all-day event must not carry a `timeZone`. RFC 5545 Section 3.2.19 explicitly specifies that `TZID` must not appear on `VALUE=DATE` properties. If a timezone is present, the event is formatted as a timed event.
  4. Duration in whole days (`whole_days`): The event duration, if present, must represent a whole number of days (e.g. `P1D`, `P7D`, `P2W`). Any duration containing sub-day units (`T`, `H`, `M`, `S`, such as `PT4H` or `P1DT1H`) disqualifies all-day emission.
  5. Sub-day recurrence time exclusion (`names_a_time_of_day`): If the event repeats, any attached recurrence rule must not include `BYHOUR`, `BYMINUTE`, or `BYSECOND` parts, which RFC 5545 Section 3.3.10 explicitly forbids beside a DATE-valued `DTSTART`. Furthermore, `UNTIL` endpoints must also fall at midnight.
  6. Recurrence override instance date-time alignment (`instance_shows_without_time`): Every override in `recurrenceOverrides` must also satisfy all-day constraints: its identifier must be at midnight, its modified start must be at midnight, its duration must be whole days, and it must not introduce a timezone.
  7. In contrast, differential oracles or simplistic serializers emit `DTSTART;VALUE=DATE` with `TZID` parameters, allow sub-day rules like `BYHOUR=10` beside DATE starts, or emit mixed DATE and DATE-TIME representations across series and detached instances.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.6.1 (`VEVENT`), Section 3.2.19 (`TZID`), Section 3.3.4 (`DATE-TIME`), Section 3.3.5 (`DATE`), Section 3.3.6 (`DURATION`), and Section 3.3.10 (`RRULE`) establish syntax rules for date-valued calendar components.
  2. RFC 8984 Section 4.1.5 (`showWithoutTime`) and Section 4.3.4 (`recurrenceOverrides`) define JSCalendar all-day event modeling. Emitting consistent date-valued properties prevents calendar backends from failing validation or displaying split-day appointments.
- **Adjudication**:
  Conforming specification boundary and all-day date serialization determinism. Enforces strict document-wide date consensus, midnight start verification, whole-day duration validation, sub-day recurrence exclusion, and override date-time alignment.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.233 Divergence 233: `read_duration`, `stated_duration`, `period_length`, `instant`, `days_from_civil`, and `to_duration`: Inbound Duration and Period Length Resolution: Explicit `DURATION` Precedence over `DTEND`, Wall-Clock Instant Measurement via Howard Hinnant's Civil Algorithm (`days_from_civil`), Signed Prefix Handling (`+` Stripping vs `-` Rejection), Loose ISO 8601 Component Ingestion (`W D H M S`), and Zero/Negative Length Fallback (`P0D` / None)

- **Observed Behavior**:
  Parsing event duration from iCalendar components into JSCalendar `duration` requires reconciling mutually exclusive properties, converting timestamps to civil day counts, and parsing flexible duration syntax. In `jmap-ical`:
  1. Explicit `DURATION` precedence over `DTEND`: In `read_duration`, if a `DURATION` property is present and valid per `stated_duration`, it is returned immediately. If absent, the parser computes the difference `DTEND - DTSTART` on the local wall clock using `instant` and `to_duration`. This accommodates Evolution's appointment editor, which writes `DTEND` rather than `DURATION`.
  2. Wall-clock instant measurement via Howard Hinnant's algorithm (`days_from_civil`): `instant` parses local date-times (`YYYYMMDDTHHMMSS`) into total elapsed seconds since 1970-01-01 using Howard Hinnant's proleptic Gregorian calendar algorithm. The algorithm treats March 1 as the start of the computational year to eliminate leap day special cases, providing exact second-level differences across centuries.
  3. Formatted duration synthesis (`to_duration`): Given elapsed seconds, `to_duration` constructs an ISO 8601 string. Whole days (multiples of 86,400 seconds) are formatted as days (`P1D`, `P2D`) without sub-day time tails, aligning with JSCalendar's nominal day semantics across daylight saving transitions. Non-day remainder seconds are formatted with `T`, `H`, `M`, and `S`. Elapsed seconds less than or equal to zero return `None`, defaulting to RFC 8984's `P0D`.
  4. Loose ISO 8601 component ingestion (`stated_duration`): `stated_duration` accepts valid durations including compound formats like `P1W2D` and `PT1H15S`. It strips redundant leading `+` signs (which RFC 5545 allows but RFC 8984 omits) and strictly rejects negative durations (such as `-PT1H`, which cannot be represented in JSCalendar).
  5. Period length resolution (`period_length`): In `RDATE;VALUE=PERIOD` entries, periods can be expressed as either a start and duration (`start/PT3H`) or a start and end (`start/end`). `period_length` validates explicit durations with `stated_duration` and evaluates explicit end timestamps via wall-clock subtraction, returning `None` for inverted or zero-length periods.
  6. In contrast, differential oracles or minimal parsers reject compound durations like `P1W2D`, produce negative durations on inverted `DTEND < DTSTART`, crash on missing `DURATION` properties, or fail to produce nominal days across DST boundaries.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.6.1 (`VEVENT`), Section 3.8.2.5 (`DURATION`), Section 3.8.2.2 (`DTEND`), Section 3.8.2.4 (`DTSTART`), Section 3.8.5.2 (`RDATE`), and Section 3.3.6 (`DURATION`) govern event duration and period definitions.
  2. RFC 8984 Section 1.4.6 (`Duration`) and Section 4.2.2 (`duration`) define JSCalendar duration representation. Calculating nominal wall-clock days ensures meeting durations do not unexpectedly contract or expand across daylight saving shifts.
- **Adjudication**:
  Conforming specification boundary and duration ingestion fidelity. Prioritizes explicit `DURATION`, measures wall-clock differences via civil day arithmetic, strips leading `+`, rejects negative durations, formats whole nominal days, and resolves period lengths.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.234 Divergence 234: `maps_alerts`, `uses_default_alerts`, `drawn_alert`, `drawn_trigger`, `is_type`, and `read_alerts`: Reminder and Alarm Gating and Transformation: User Default Alert Masking (`useDefaultAlerts`), Closed Action Vocabulary (`action: "display"` / `ACTION:DISPLAY`), Offset Trigger Normalization (`OffsetTrigger` with `START` / `END` `RELATED`), RFC 9074 Stable Alert `UID` Ingestion, and Non-Colliding Invented Key Generation (`a1`, `a2`)

- **Observed Behavior**:
  Translating reminder alarms between JSCalendar `alerts` and RFC 5545 `VALARM` components requires action vocabulary validation, trigger type checking, stable identity binding, and default reminder suppression. In `jmap-ical`:
  1. User default alert masking (`uses_default_alerts`): RFC 8984 Section 4.5.1 provides `useDefaultAlerts`. When `useDefaultAlerts: true`, explicit `alerts` are ignored and the user's client preferences fire instead. `maps_alerts` checks `uses_default_alerts(event)`: if true, `maps_alerts` returns `false` and `drawn_alarms` suppresses all `VALARM` components, preventing the generation of redundant alarms that the user never intended to set.
  2. Closed action vocabulary (`action: "display"`): RFC 5545 Section 3.6.6 defines `DISPLAY`, `AUDIO`, and `EMAIL` alarms. RFC 8984 Section 4.5.2 restricts Alert actions to `display`. `drawn_alert` verifies that an alert's action is `display` (or omitted, which defaults to display). Other actions (such as email or audio) are rejected because they require additional parameters (like `SUMMARY` and `ATTENDEE` on email alarms) not modeled in JSCalendar.
  3. Offset trigger normalization (`drawn_trigger`): Only RFC 8984 Section 4.5.3 `OffsetTrigger` is admitted; absolute triggers (`AbsoluteTrigger`, Section 4.5.4) are rejected because guessing an offset from a fixed instant would cause the alarm to drift whenever the event is rescheduled. `drawn_trigger` parses signed durations via `stated_offset`. For `relativeTo: "end"`, it attaches `RELATED=END`. For default `relativeTo: "start"`, it omits the parameter, matching RFC 5545 Section 3.2.14 defaults.
  4. RFC 9074 stable alert `UID` ingestion and invented key generation (`read_alerts`): When reading `VALARM` components, `read_alerts` extracts RFC 9074 Section 6 `UID` properties. If a `UID` is present, it is used directly as the map key in `alerts`. For alarms lacking a `UID` (such as those generated by Evolution with `X-EVOLUTION-ALARM-UID`), `read_alerts` synthesizes positional invented keys (`a1`, `a2`, ...) in a loop that explicitly checks for and avoids colliding with any existing keys in the map.
  5. In contrast, differential oracles or naive parsers attempt to map unsupported email/audio alarms, approximate absolute triggers as fixed offsets, omit mandatory RFC 9074 `UID` lines on serialization, or assign colliding map keys on import.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.6.6 (`VALARM`), Section 3.8.6.1 (`ACTION`), Section 3.8.6.3 (`TRIGGER`), and Section 3.2.14 (`RELATED`) define alarm component syntax. RFC 9074 Section 6 defines `UID` on `VALARM`.
  2. RFC 8984 Section 4.5.1 (`useDefaultAlerts`), Section 4.5.2 (`alerts`), Section 4.5.3 (`OffsetTrigger`), and Section 4.5.4 (`AbsoluteTrigger`) define JSCalendar reminder models.
- **Adjudication**:
  Conforming specification boundary and reminder alarm translation fidelity. Enforces `useDefaultAlerts` masking, validates closed `display` action vocabulary, normalizes offset triggers with canonical `RELATED` parameters, preserves RFC 9074 `UID` identifiers, and generates collision-free synthetic keys.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.235 Divergence 235: `maps_recurrence_override`, `sends_recurrence_override`, `override_maps_by`, `maps_override_field`, and `draws_override_field`: Recurrence Override Modification Safety Gating: Strict Property Whitelisting (`OVERRIDE_PROPERTIES`), Isolated Override vs Coordinated TimeZone Definition Emission (`maps_override_field` vs `draws_override_field`), Set Replacement Semantics (`keywords` and `alerts`), and Exclusion Purity (`excluded: true`)

- **Observed Behavior**:
  Validating and serializing recurrence override patches (`recurrenceOverrides`) into detached RFC 5545 `VEVENT` components requires property whitelisting, exclusion purity verification, timezone scoping coordination, and set replacement handling. In `jmap-ical`:
  1. Strict property whitelisting (`OVERRIDE_PROPERTIES`): An override patch may only modify the 11 properties in `OVERRIDE_PROPERTIES` (`title`, `description`, `start`, `timeZone`, `duration`, `status`, `freeBusyStatus`, `priority`, `privacy`, `keywords`, `alerts`). Any patch attempting to modify non-overridable properties (such as `participants` or `locations`, which cannot be cleanly restated in detached instances without losing server state) fails validation.
  2. Exclusion purity (`excluded: true`): An override that excludes an occurrence (`"excluded": true`) must contain no other properties. If a patch contains `excluded: true` alongside modifications like `title` or `start`, `override_maps_by` rejects the patch, preventing conflicting state where an occurrence is marked as both canceled and rescheduled.
  3. Isolated override vs coordinated timezone emission (`maps_override_field` vs `draws_override_field`): In an isolated `recurrenceOverrides` patch, `maps_override_field` restricts `timeZone` to standard IANA timezone names (`names_time_zone`). A custom solidus-prefixed timezone (e.g. `/custom-zone`) is refused because an isolated patch has no mechanism to attach the required `timeZones` definition. Conversely, in a coordinated save or during whole-event drawing, `draws_override_field` (and `sends_recurrence_override`) admits custom timezones if they are defined in the series' `timeZones` map.
  4. Set replacement semantics (`keywords` and `alerts`): In JSCalendar override patches, set properties (`keywords` and `alerts`) replace the inherited series set rather than merging with it. A null value (`null`) clears the set. However, an empty object (`{}`) is explicitly refused by `maps_override_field` because an empty map serializes identically to a removal (carrying no line), which would cause a round-trip divergence.
  5. In contrast, differential oracles or permissive parsers permit non-overridable properties in instance patches, allow contradictory properties on excluded occurrences, emit dangling custom timezone references, or misinterpret empty set patches.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.8.4.4 (`RECURRENCE-ID`), Section 3.8.5.1 (`EXDATE`), Section 3.8.5.2 (`RDATE`), and Section 3.6.1 (`VEVENT`) define recurring series and instance components.
  2. RFC 8984 Section 4.3.4 (`recurrenceOverrides`), Section 1.4.9 (`TimeZoneId`), and Section 4.7.2 (`timeZones`) define JSCalendar instance overrides and timezone coordination.
- **Adjudication**:
  Conforming specification boundary and recurrence override safety gating. Enforces `OVERRIDE_PROPERTIES` whitelisting, verifies exclusion purity, coordinates custom timezone definitions with series envelopes, and maintains deterministic set replacement semantics.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.
...
### 13.328 Divergence 328: `parse_ical`, `check_structure`, `BEGIN:VCALENDAR`, `VERSION:2.0`, `fold_overlong_lines`, and `CRLF`: Inbound iCalendar Stream Syntactic Framing, Document Boundary Enforcement, Nested Component Tree Depth Bounding, and RFC 5545 Section 3.1 75-Octet Line Folding Safety

- **Observed Behavior**:
  Parsing raw RFC 5545 calendar streams into structured AST containers requires validating outermost document boundaries, verifying mandatory `VERSION:2.0` indicators, balancing nested `BEGIN`/`END` component tags, and safely handling line folding across multi-byte character sequences. In `jmap-ical`:
  1. Root envelope enforcement (`parse_ical`): Evaluates unfolded content lines to verify that the stream begins with `BEGIN:VCALENDAR` and closes with `END:VCALENDAR`. Headless streams containing bare `VEVENT` blocks without an enclosing `VCALENDAR` envelope return `Err(ICalError::NotACalendar)`, preventing fragmented components from leaking into the parser state.
  2. Component tag balancing (`check_structure`): Tracks open component tags on an internal stack across unfolded lines. If an `END:<name>` statement mismatches the innermost open tag (e.g. `BEGIN:VEVENT` closed by `END:VTODO`), it halts immediately with `Err(ICalError::Mismatched { expected, found })`. If unclosed components remain at EOF, it returns `Err(ICalError::Unterminated(name))`.
  3. Byte Order Mark stripping: Pre-scans the raw input string and cleanly strips leading UTF-8 Byte Order Marks (`\u{feff}`), preventing Windows calendar exports from failing root envelope detection.
  4. Nesting depth bounding (`check_depth`, `MAX_DEPTH = 32`): Recursively walks component child relationships to verify that nested components do not exceed 32 levels, preventing stack exhaustion denial-of-service attacks from maliciously nested structures.
  5. 75-octet line folding (`fold_overlong_lines`): RFC 5545 Section 3.1 mandates that physical lines in iCalendar streams SHOULD NOT exceed 75 octets. `fold_overlong_lines` splits overlong content lines by inserting CRLF followed by a linear whitespace space character (`\r\n `), ensuring that multi-byte UTF-8 character sequences are never split across line boundaries.
  6. In contrast, differential oracles or liberal parsers may accept headless or versionless streams, allow unclosed components to leak into adjacent records, or fail on multi-line unfolded UTF-8 sequences.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.1 (`Content Lines`), Section 3.4 (`Compatibility`), Section 3.6 (`Calendar Components`), and Section 3.7.4 (`Version`).
  2. RFC 8984 Section 1.5 (`Serialization of JSCalendar Objects`).
- **Adjudication**:
  Conforming specification boundary and syntactic framing safety. Mandates balanced `VCALENDAR` component envelopes, enforces finite nesting depth limits, strips leading UTF-8 BOMs, and provides RFC 5545 Section 3.1 75-octet line folding with UTF-8 boundary protection.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.329 Divergence 329: `entry_text`, `entry_texts`, `entry_raw_value`, `entry_param`, `entry_param_values`, `component_entry`, `component_entries`, and `component_text`: Calcard AST Component Traversal and Value Extraction: Case-Insensitive Property/Parameter Key Matching, Comma-Delimited Multi-Value Parsing, Structured Text Value Flattening, and Parameter Value Multi-Set Aggregation

- **Observed Behavior**:
  Navigating raw iCalendar syntax nodes generated by `calcard` requires robust AST traversal primitives that tolerate case variation, extract typed parameters, and flatten multi-value property lines. In `jmap-ical`:
  1. Case-insensitive component lookup (`component_entry`, `component_entries`, `component_text`): RFC 5545 Section 3.1 states that property and parameter names are case-insensitive. `component_entry` and `component_entries` match entry names using `eq_ignore_ascii_case`, successfully resolving uppercase (`SUMMARY`), lowercase (`summary`), and mixed-case properties emitted by diverse exporters.
  2. Multi-occurrence property aggregation (`component_entries`): Collects all entries sharing the specified name, enabling comprehensive multi-line ingestion for repeated properties such as `RRULE`, `ATTACH`, `CATEGORIES`, or `CONFERENCE`.
  3. Raw and unescaped value extraction (`entry_text`, `entry_raw_value`): `entry_raw_value` retrieves the exact underlying string payload (essential for URIs and date-time stamps), while `entry_text` unescapes RFC 5545 character sequences (`\,`, `\;`, `\\`, and newline literals `\n`/`\N`) into native Unicode strings.
  4. Case-insensitive parameter extraction (`entry_param`, `entry_param_values`): Retrieves parameters regardless of casing (`entry_param(entry, "fmttype")` matches `FMTTYPE=application/pdf`). For parameters that may appear multiple times on a single line (such as `CONFERENCE;FEATURE=AUDIO;FEATURE=VIDEO`), `entry_param_values` aggregates all values into a consolidated vector.
  5. In contrast, differential oracles or simplistic AST mappers perform case-sensitive lookups (dropping lowercase properties), fail on repeated parameter keys, or lose unescaped newline delimiters.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.1 (`Content Lines`), Section 3.2 (`Property Parameters`), and Section 3.3.11 (`Text`).
  2. RFC 8984 Section 1.4 (`Type Signatures`).
- **Adjudication**:
  Conforming specification boundary and AST extraction fidelity. Provides case-insensitive property and parameter lookups, aggregates multi-occurrence parameter entries, and unescapes text strings cleanly.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.330 Divergence 330: `read_duration`, `stated_duration`, `period_length`, `instant`, `to_duration`, and `days_from_civil`: Event Duration and Interval Resolution Pipeline: Explicit `DURATION` Precedence over `DTEND`, Gregorian Timeline Instant Measurement (Hinnant Civil Algorithm), ISO 8601 Component Ingestion (`P<n>DT<n>H<n>M<n>S`), and Negative/Zero Interval Containment

- **Observed Behavior**:
  Resolving event duration across RFC 5545 (which allows explicit `DURATION` or calculated `DTEND - DTSTART`) and JSCalendar RFC 8984 Section 4.1.6 `duration` requires strict interval validation, precedence rules, and deterministic calendar arithmetic. In `jmap-ical`:
  1. Explicit `DURATION` precedence (`read_duration`): RFC 5545 Section 3.6.1 permits either `DTEND` or `DURATION` to define an event interval. When both properties are present, `read_duration` prioritizes the explicit `DURATION` line, adhering to the author's stated duration rather than relying on computed endpoints.
  2. Computed duration fallback (`instant`, `to_duration`): When `DURATION` is absent, `read_duration` calculates the interval from `DTEND` and `DTSTART`. It parses both timestamps via `instant` (which maps dates to seconds since the civil epoch via Howard Hinnant's algorithm) and computes the difference via `to_duration(end - start)`.
  3. Negative and inverted interval rejection: If `DTEND` precedes `DTSTART` (producing negative elapsed seconds), `to_duration` returns `None`. Similarly, `stated_duration` rejects negative durations (`-PT1H`), ensuring that inverted intervals fail closed rather than corrupting JSCalendar models with invalid negative durations.
  4. Stated duration normalization (`stated_duration`): Accepts ordered ISO 8601 duration components (`W D T H M S`). A leading `+` prefix is stripped, while malformed expressions or strings containing out-of-order designators return `None`.
  5. Period length resolution (`period_length`): In `RDATE;VALUE=PERIOD` entries, periods can be specified either as an explicit duration (`start/PT3H`) or as a start and end pair (`start/end`). `period_length` evaluates explicit durations via `stated_duration` and computes timestamp pairs via wall-clock second deltas.
  6. Pure Gregorian civil timeline arithmetic (`days_from_civil`): Computes elapsed civil days from 1970-01-01 using Howard Hinnant's proleptic Gregorian calendar algorithm, correctly handling leap years (including 400-year century exceptions) without external runtime dependencies.
  7. In contrast, differential oracles accept negative durations, fail to normalize multi-unit durations, or crash on century leap-year boundaries.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.3.6 (`Duration`), Section 3.3.9 (`Period of Time`), Section 3.6.1 (`Event Component`), Section 3.8.2.2 (`Date-Time End`), and Section 3.8.2.5 (`Duration`).
  2. RFC 8984 Section 1.4.6 (`Duration`) and Section 4.1.6 (`duration`).
- **Adjudication**:
  Conforming specification boundary and duration arithmetic determinism. Prioritizes explicit `DURATION` over `DTEND`, implements exact Gregorian epoch calculations, validates ISO 8601 duration structures, and rejects negative intervals.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.

### 13.331 Divergence 331: `read_links`, `drawn_links`, `drawn_link`, `fetched_locally`, `stated_size`, and `media_type`: External Link and Attachment Ingestion and Serialization Pipeline: RFC 8984 Section 4.2.7 `Link` Object Synthesis, `ATTACH` vs `IMAGE` Disambiguation, `file:` URI Local Attachment Privacy Defense, RFC 8607 `SIZE` Parameter Extraction, and Deterministic Map Key Assignment

- **Observed Behavior**:
  Translating external attachments and image badges between RFC 5545 / RFC 7986 and JSCalendar RFC 8984 Section 4.2.7 requires mapping `ATTACH` and `IMAGE` properties into structured `Link` objects, protecting local filesystem paths, and preserving map keys. In `jmap-ical`:
  1. Attachment and image property disambiguation (`read_links`): Ingests both `ATTACH` (RFC 5545 Section 3.8.1.1) and `IMAGE` (RFC 7986 Section 5.10). For `IMAGE`, it automatically sets `rel: "icon"` and translates the `DISPLAY` parameter to JSCalendar `display` via `LINK_DISPLAYS` (`BADGE`, `GRAPHIC`, `FULLSIZE`, `THUMBNAIL`). For `ATTACH`, it extracts `FMTTYPE` to `contentType` and parses RFC 8607 `SIZE` parameters as unsigned 64-bit integers (`u64`).
  2. Local file URI privacy protection (`fetched_locally`): Evolution and local desktop calendar tools frequently attach local files using `file:///...` URIs during draft creation. `read_links` checks each URI with `fetched_locally(&href)`. Any local `file:` URI is dropped immediately during inbound parsing, preventing workstation file paths from leaking to remote calendar participants or across synchronization boundaries.
  3. Stable key preservation and synthetic fallback: Preserves incoming `X-JMAP-KEY` parameters on attachment lines when valid (`names_map_entry`). When absent, it synthesizes deterministic keys (`k1`, `k2`) rather than random UUIDs, avoiding churn across repeated imports.
  4. Outbound link serialization (`drawn_links`, `drawn_link`): Translates `Link` objects back to iCalendar properties. Links with `rel: "icon"` are emitted as `IMAGE;VALUE=URI` with `DISPLAY`, `FMTTYPE`, and `X-JMAP-KEY`. Standard links (or `rel: "enclosure"`) are emitted as `ATTACH` with `SIZE`, `FMTTYPE`, and `X-JMAP-KEY`.
  5. In contrast, differential oracles emit local file URIs unchecked, conflate image display modes, or fail to parse RFC 8607 attachment sizes.
- **Specification and Architectural Context**:
  1. RFC 5545 Section 3.8.1.1 (`Attachment`) and Section 3.2.8 (`Format Type`).
  2. RFC 7986 Section 5.10 (`Image`) and Section 6.1 (`Display Parameter`).
  3. RFC 8607 Section 4.1 (`Size Parameter`).
...
- **Adjudication**:
  Conforming specification boundary and external link handling fidelity. Disambiguates `ATTACH` and `IMAGE` properties, defends against local `file:` path leakage, parses RFC 8607 sizes, and preserves map keys across round-trips via `X-JMAP-KEY`.
- **Status**:
  Conforming specification boundary. Documented and pinned in `tests/event.rs`.
