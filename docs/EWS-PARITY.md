...
## Surface 8 - Mail store/folder vfuncs (`camel-ews-store.c`/`camel-ews-folder.c` vs `jmap-mail`)

The one surface not yet compared: the code path every mail read and write
actually goes through. `CamelEwsStore`/`CamelEwsFolder` are plain
`CamelStore`/`CamelFolder` subclasses (no meta-backend layer the way the
calendar and address-book backends have), so this table lists every vfunc
slot each class installs. Upstream source is read from
`src/EWS/camel/camel-ews-store.c` and `src/EWS/camel/camel-ews-folder.c` on
`master`; this project's file:line references are relative to `rust/crates/`.
The base-class defaults cited (`camel_folder_get_message_cached`,
`camel_folder_search_body_sync`, `camel_store_initial_setup_sync`, and so on)
are read from evolution-data-server's own `src/camel/camel-folder.c` and
`src/camel/camel-store.c`, `master`, since a Camel subclass is judged against
what its base class already does for a slot it leaves unfilled, not only
against EWS.

### `CamelServiceClass` and `CamelStoreClass` (store)

| EWS vfunc/behavior | What it does | Our equivalent | Verdict | Reasoning |
|---|---|---|---|---|
| `query_auth_types_sync` | Probes the server and returns the `CamelServiceAuthType` list the legacy account-setup wizard offers (`camel-ews-store.c:2236-2315`) | Absent; `jmap-mail/src/service.rs:356`'s own comment states the decision: "Basic and Bearer are HTTP, negotiated by the client... which is also why `query_auth_types_sync` is left alone" | DIVERGENCE - justified | JMAP account setup goes through this project's own `jmap-config` discovery module (`ESourceConfigLookup`), not the legacy per-store auth-type list; there is no server-advertised set of SASL-like mechanisms to enumerate the way Exchange's NTLM/Basic/OAuth choices are. |
| `connect_sync` | Chains to the parent, then confirms the connection status (`camel-ews-store.c:1422-1638`) | `connect_sync::<T>`, `jmap-mail/src/service.rs:302` | MATCH | Both are a short-circuit to the session's `authenticate_sync` re-prompt loop; see the module's own docs on why. |
| `disconnect_sync` | Drops the connection under a lock, chains to the parent (`camel-ews-store.c:1639-1653`) | `disconnect_sync::<T>`, `jmap-mail/src/service.rs:513` | MATCH | Same shape: release the held connection, then the parent's own teardown. |
| `authenticate_sync` | Reads the password, opens the connection, and on first setup also diffs the whole folder tree inline (creates/updates/deletes) before returning (`camel-ews-store.c:2048-2236`) | `authenticate_sync::<T>`, `jmap-mail/src/service.rs:375`, delegating to `crate::service::authenticate` and `crate::connect::open_mail` | MATCH (pattern) / justified divergence | Both open the account and install the connection. EWS folds a folder-tree diff into this call because `SyncFolderItems`/`GetFolder` need the connection it just opened; this store's folder listing is a separate, lazy `Mailbox/get` in `crate::folders`, so nothing here needs to pre-populate it. |
| `get_name` | Formats `"Exchange server %s"` (brief) or a longer sentence, from settings (`camel-ews-store.c:3352-3370`) | `get_name`, `jmap-mail/src/service.rs:248`, delegating to `describe` | MATCH | Same two-form contract, same settings-only inputs. |
| `initial_setup_sync` | On first connect, fetches Exchange's distinguished folders and reports the account's Sent and Drafts folders back to Evolution via `save_setup` (`camel-ews-store.c:1924-2047`) | Absent | **GAP** | RFC 8621 gives a mailbox a `role`, and this crate already resolves `FolderRole::Sent`/`FolderRole::Drafts` elsewhere (`jmap-mail-sync/src/send.rs:221-223`) for exactly this purpose. Without this vfunc, and without the store ever setting `CAMEL_STORE_SUPPORTS_INITIAL_SETUP` on itself, Evolution's account wizard silently skips folder auto-detection entirely (`mail-folder-cache.c:2000` in evolution's own tree treats a store lacking the flag as trivially successful) and never even shows the "Lookup Folders" button (`e-mail-config-defaults-page.c:820`) a user could retry by hand. A fresh JMAP account's Sent/Drafts identity settings are left to whatever default Evolution otherwise falls back to, not the account's actual roled mailboxes, the same class of bug `get_inbox_folder_sync` was written to avoid for the inbox. |
| `get_folder_sync` | Confirms the folder exists in the summary, opens it (`camel-ews-store.c:2316-2339` onward) | `get_folder_sync`, `jmap-mail/src/folders.rs:434` | MATCH | Both resolve a path against the held folder listing before building the `CamelFolder`. |
| `create_folder_sync` | Rejects a name collision, creates via `CreateFolder`, updates the summary, emits (`camel-ews-store.c:2839` onward) | `create_folder_sync`, `jmap-mail/src/manage.rs:265` | MATCH | Same shape; `Mailbox/set` create in place of `CreateFolder`. |
| `delete_folder_sync` | Looks up the folder info, deletes via `DeleteFolder`, updates the summary, emits (`camel-ews-store.c:2970` onward) | `delete_folder_sync`, `jmap-mail/src/manage.rs:313` | MATCH | Same shape; `Mailbox/set` destroy. |
| `rename_folder_sync` | Splits old/new paths, renames via `MoveFolder`/`UpdateFolder`, updates the summary (`camel-ews-store.c:3160` onward) | `rename_folder_sync`, `jmap-mail/src/manage.rs:359` | MATCH | Same shape; see `manage.rs`'s own module doc for how a Camel path is split back into a JMAP parent and display name. |
| `get_folder_info_sync` | Refreshes if requested, then builds the `CamelFolderInfo` forest from the summary (`camel-ews-store.c:2641` onward) | `get_folder_info_sync`, `jmap-mail/src/folders.rs:375` | MATCH | Same contract, including honoring `RECURSIVE` and the two subscription flags; see `folders.rs`'s own module doc for why `SUBSCRIBED`/`SUBSCRIPTION_LIST` need no second round trip here. |
| `get_inbox_folder_sync` | Resolves the account's Inbox by Exchange's distinguished folder id (`camel-ews-store.c:3429-3437`) | `get_inbox_folder_sync`, `jmap-mail/src/folders.rs:506`, via `open_by_role` | MATCH (pattern) / justified divergence | Both resolve by a server-assigned purpose rather than a name; RFC 8621's `role` is this store's equivalent of Exchange's distinguished id. |
| `get_trash_folder_sync` | Resolves Trash by role, then synchronizes every other open folder and force-refreshes Trash so a just-deleted message shows immediately (`camel-ews-store.c:3440-3467`) | `get_trash_folder_sync`, `jmap-mail/src/folders.rs:544`, via `open_by_role` | MATCH (pattern) / justified divergence | The extra synchronize-then-refresh dance exists because Exchange's own deletion is asynchronous from the client's point of view; this store's `Email/set` that files a message into Trash has already returned by the time `expunge_sync` does, so there is nothing stale left for a forced refresh to catch up on. |
| `get_junk_folder_sync` | Resolves Junk by distinguished id (`camel-ews-store.c:3479-3487`) | `get_junk_folder_sync`, `jmap-mail/src/folders.rs:567`, via `open_by_role` | MATCH | Same reasoning as Inbox above, and `$junk` is an actual JMAP keyword unlike the deleted case. |
| `can_refresh_folder` | Checks a `check-all` setting and a per-folder `check-folder` property, falling back to the parent (`camel-ews-store.c:3489-3529`) | `can_refresh_folder`, `jmap-mail/src/folders.rs:341`, via `refreshable` | MATCH (pattern) / justified divergence | `Mailbox/get`'s `isSubscribed` already answers "does the user want this folder checked" in the one call the listing already made; an IMAP-style settings knob would be answering a question this protocol already answered. |
| `CamelSubscribableInterface` (`folder_is_subscribed`/`subscribe_folder_sync`/`unsubscribe_folder_sync`) | Backed by the store's own summary flags for foreign/public folders (`camel-ews-store.c:3531` onward) | `Subscribable`, `jmap-mail/src/subscribe.rs:124`, backed by `Mailbox/set`'s `isSubscribed` | MATCH | Same three-method contract; see `subscribe.rs`'s own module doc for the non-blocking-read and emission rules both providers follow. |
| `GObjectClass::dispose` | Disconnects two settings-signal handlers this store installed elsewhere, saves the summary, drops the connection (`camel-ews-store.c:4035-4064`) | Absent; only `finalize` is overridden (`jmap-mail/src/store.rs:1487`) | DIVERGENCE - justified | This crate connects no `g_signal_connect` handlers anywhere (confirmed by grep across the whole crate), so there is nothing a `dispose` would need to disconnect; `finalize` alone is what every other object-subclass type in this crate relies on for the same reason. |
| `GObjectClass::finalize` | Frees the storage path and clears three locks (`camel-ews-store.c:4068-4084`) | `finalize`, `jmap-mail/src/store.rs:1487-1507` | MATCH (pattern) | Both release what construction acquired; see `store.rs`'s own comments on why the push/refresher slots are cleared before the connection. |

### `CamelFolderClass` (folder)

| EWS vfunc/behavior | What it does | Our equivalent | Verdict | Reasoning |
|---|---|---|---|---|
| `get_permanent_flags` | Returns a fixed set (`ANSWERED\|DELETED\|DRAFT\|FLAGGED\|SEEN\|FORWARDED\|USER`) (`camel-ews-folder.c:1141-1150`) | Absent; inherits the base class default of `0` (`camel-folder.c:927-930`) | DIVERGENCE - worth a second look | Every provider checked (EWS, IMAPX, and the local maildir/mbox providers) overrides this with a non-zero answer including `CAMEL_MESSAGE_USER`, and this crate supports the same flags and arbitrary keywords. But `camel_folder_get_permanent_flags` has no caller anywhere in evolution's or evolution-data-server's own trees outside of IMAPX's own internal `PERMANENTFLAGS` wire-state bookkeeping, so the practical impact of leaving it at `0` could not be confirmed either way from source alone. |
| `get_message_sync` | Fetches from cache or the server, updates the mailing-list header cache (`camel-ews-folder.c:1152-1167`) | `get_message_sync`, `jmap-mail/src/message.rs:93` | MATCH | Same cache-then-network shape; see `message.rs`'s own module doc. |
| `get_message_cached` | A lock-free cache-only lookup, delegating to the same on-disk cache `get_message_sync` falls back to (`camel-ews-folder.c:1171-1176`) | Absent | **GAP** | `camel_folder_get_message_sync`'s own wrapper (`camel-folder.c:3471`) always tries `get_message_cached` first, *before* taking `camel_folder_lock`, and only falls through to the locked `get_message_sync` vfunc on a miss. This crate's `get_message_sync` (`message.rs:113-124`) already does an equivalent local-cache check via `crate::cache`, but only after the folder lock is held, so every message open, cached or not, waits on a lock a genuinely cache-hit read would not need to. Filling this slot with a thin wrapper around the same `JmapFolder::cache()`/`cache.load()` pair `get_message_sync` already calls would close the gap. |
| `cmp_uids` | `strcmp` over the two uids (`camel-ews-folder.c:3049-3056`) | Absent; inherits the base default, `g_ascii_strtoull` numeric compare (`camel-folder.c:1005-1013`) | DIVERGENCE - worth a second look | JMAP `Email` ids are opaque strings, not numeric, so the inherited default's numeric parse is meaningless for this store's uids. Every reference provider overrides it with a comparison suited to its own uid scheme (EWS and IMAPX with `strcmp`, maildir/mbox with their own). No caller of `camel_folder_cmp_uids`/`camel_folder_sort_uids` was found in this crate, though, so whether an unfilled slot is ever actually reached here could not be confirmed. |
| `append_message_sync` | Serializes the message, uploads via `CreateItem` (`camel-ews-folder.c:2514` onward) | `append_message_sync`, `jmap-mail/src/append.rs:112` | MATCH | Same shape; `Email/import` in place of `CreateItem`. |
| `refresh_info_sync` | Lists changes via `SyncFolderItems`, reconciles the summary, emits `changed` (`camel-ews-folder.c:2276-2513`) | `refresh_info_sync`, `jmap-mail/src/refresh.rs:119` | MATCH | Same shape; see `refresh.rs`'s own module doc on why the folder asks the store rather than holding a connection itself. |
| `synchronize_sync` | Walks the summary's changed rows, writes flag changes back via `UpdateItem` (`camel-ews-folder.c:1586-1717`) | `synchronize_sync`, `jmap-mail/src/synchronize.rs:128` | MATCH | Same shape; `Email/set` keyword patch in place of `UpdateItem`, walking the same `camel_folder_summary_get_changed` work list IMAPX also drives from. |
| `expunge_sync` | Deletes items carrying `CAMEL_MESSAGE_DELETED` via `DeleteItem` (`camel-ews-folder.c:2974-3048`) | `expunge_sync`, `jmap-mail/src/expunge.rs:103` | MATCH (pattern) / justified divergence | Both act on the same local-only Camel flag; the write each issues differs because JMAP models removal as a `mailboxIds` change per message rather than one folder-wide delete call, per `expunge.rs`'s own module doc. |
| `transfer_messages_to_sync` | Moves or copies via `MoveItem`/`CopyItem`, one request per message (`camel-ews-folder.c:2596-2773`) | `transfer_messages_to_sync`, `jmap-mail/src/transfer.rs:133` | MATCH | Same per-message request shape; `Email/set` `mailboxIds` patch in place of `MoveItem`/`CopyItem`. |
| `prepare_content_refresh` | Clears the folder's stored sync state so the next refresh is a full re-list rather than a delta (`camel-ews-folder.c:2774-2779`) | Absent | DIVERGENCE - justified | This store's own equivalent of a stale sync state is JMAP's `cannotCalculateChanges` answer from `Email/changes`, which `jmap-mail-sync::messages_since` already detects and answers by relisting the mailbox (`jmap-mail-sync/src/lib.rs:268`). A client-side "throw away the cursor" button solves a problem JMAP already reports server-side; EWS needs one because Exchange's `SyncFolderItems` state is opaque to the client until it fails. |
| `get_filename` | SHA-256 of the uid, looked up in the on-disk `CamelDataCache` (`camel-ews-folder.c:165-179`) | `get_filename`, `jmap-mail/src/cache.rs:537` | MATCH | Same scheme, same reasoning: a bare uid is not a safe filename component. |
| `search_body_sync` | Server-side `body-contains` search via `FindItem`, one request per folder (`camel-ews-folder.c:3060-3167`) | Absent; inherits the base class default, a local `CamelIndex` lookup that answers `G_IO_ERROR_NOT_SUPPORTED` when no such index exists (`camel-folder.c:1482-1560`), which no code in this crate ever builds | **GAP** | This is the same server-side "body contains" search this crate already performs for the *pre-3.58* search path: `search_sexp::translate` (`search_sexp.rs:56`) turns exactly this kind of condition into an `Email/query` filter, reached through `search_by_expression` under `#[cfg(camel_folder_search_object)]` (`folder.rs:287-292`). But `search_body_sync` is the slot EDS's newer `CamelStoreSearch` engine (`camel-store-search.c:3945`, the base-class `search_sync` that replaces `search_by_expression`/`search_by_uids` from EDS 3.58 onward) calls for the same kind of term, and this crate never fills it. So on an EDS built against the newer search engine, a body-contains search term against a JMAP folder silently returns no matches (the base default's `NOT_SUPPORTED` is treated as "no match" by the caller, not surfaced as an error) instead of reaching the server the way it already does on EDS 3.52. |
| `GObjectClass::constructed` | Sets the folder's `description` property to `"user@host:full_name"` for display (`camel-ews-folder.c:3262-3300`) | Absent | DIVERGENCE - justified / minor | `camel_folder_get_description`'s own default falls back to the folder's full name when no description is set (`camel-folder.c:2169-2176`), which is already a meaningful answer. IMAPX, the closer analog for a network-backed provider, does not override `constructed` for this either; only EWS and the local maildir/mbox providers bother. |
| `GObjectClass::dispose`/`finalize` | Disconnects a summary signal handler and drops the on-disk cache reference in `dispose`; clears locks and the fetch state in `finalize` (`camel-ews-folder.c:3225-3260`) | Only `finalize`, clearing the mailbox id and cache slots (`jmap-mail/src/folder.rs:301-310`) | DIVERGENCE - justified | Same reasoning as the store's own dispose/finalize split above: this crate connects no signal handlers to disconnect, so there is nothing a `dispose` needs to do that `finalize` does not already cover. |

**Three real gaps found**, none EWS-specific, all already have the data or the machinery they need sitting elsewhere in this crate:

1. **`initial_setup_sync` is not implemented and the store never sets `CAMEL_STORE_SUPPORTS_INITIAL_SETUP`**, so Evolution's account wizard never pre-selects the account's real Sent/Drafts mailboxes by role the way it already does for Inbox/Trash/Junk, and never even offers the manual "Lookup Folders" retry. `FolderRole::Sent`/`FolderRole::Drafts` are already resolved elsewhere in this crate (`jmap-mail-sync/src/send.rs:221-223`).
2. **`get_message_cached` is not implemented**, so every message open takes `camel_folder_lock` even when the answer is a pure cache hit that needs no network access at all; this crate's own `get_message_sync` already contains the cache lookup this slot would delegate to.
3. **`search_body_sync` is not implemented**, so a body-contains search term against a JMAP folder silently finds nothing on an EDS built against the newer (3.58+) `CamelStoreSearch` search engine, even though this crate already translates the identical condition into a working server-side `Email/query` for the older `search_by_expression` path.

Two further divergences (`get_permanent_flags`'s `0` vs. every reference provider's non-zero answer, and the inherited numeric `cmp_uids` against JMAP's opaque string uids) could not be confirmed as having any observable effect: no caller of either was found anywhere in evolution's or evolution-data-server's own trees beyond IMAPX's own internal bookkeeping, so they are recorded as open questions rather than gaps.
...
The mail store/folder surface (Surface 8) is the code path every mail read
and write actually goes through, and it shows the same close parity as the
